...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Store submitted name in session
    $_SESSION['relative_first_name'] = $_POST['relative-first-name'] ?? '';
    $_SESSION['relative_middle_name'] = $_POST['relative-middle-name'] ?? '';
    $_SESSION['relative_last_name'] = $_POST['relative-last-name'] ?? '';
...
<body>
    ...
    <div class="birth-container">
        <div class="form-title">
            <h3>Please indicate your
                <?= htmlspecialchars($relation) ?>'s name on certificate of death.
            </h3>
        </div>
        <form method="post">
            <div class="form-p">
                <p>
                    <?= htmlspecialchars($relation) ?>'s Information
                </p>
            </div>
            <div class="form-group">
                <label class="input-label">Enter Your
                    <?= htmlspecialchars($relation) ?>'s First Name:
                </label>
                <input class="input-value" type="text" id="first-name" placeholder="First name"
                    name="relative-first-name">
            </div>
            <div class="form-group">
                <label class="input-label">Enter Your
                    <?= htmlspecialchars($relation) ?>'s Middle Name:
                </label>
                <input class="input-value" type="text" id="middle-name" placeholder="Middle name"
                    name="relative-middle-name"><br>
            </div>
            <div class="form-group">
                <label class="input-label">Enter Your
                    <?= htmlspecialchars($relation) ?>'s Last Name:
                </label>
                <input class="input-value" type="text" id="last-name" placeholder="Last name"
                    name="relative-last-name"><br>
            </div>
            <div class="submit-btn-birth">
                <button id="primary-back-btn" type="button">Back</button>
                <button id="primary-btn" type="submit">Continue</button>
            </div>
        </form>
    </div>
    <footer class="footer">
