...
<body>
    ...
    <div class="review-container">
        ...
        <div class="review-content">
            ...
            <div class="personal-info">
                <h3>Someone Information</h3>
                <p><strong>Sex:</strong> <?= htmlspecialchars($_SESSION['their-sex']) ?></p>
                <p><strong>First Name:</strong> <?= htmlspecialchars($_SESSION['their-first-name']) ?></p>
                <p><strong>Middle Name:</strong> <?= htmlspecialchars($_SESSION['their-middle-name']) ?></p>
                <p><strong>Last Name:</strong> <?= htmlspecialchars($_SESSION['their-last-name']) ?></p>
                <?php if (strtolower($_SESSION['their-sex']) === 'female'): ?>
                    <p><strong>Marital Status:</strong> <?= htmlspecialchars($_SESSION['their-marital-status']) ?></p>
                    <p><strong>Married Last Name:</strong> <?= htmlspecialchars($_SESSION['their-married-last-name']) ?></p>
                <?php endif; ?>
                <p><strong>Birthday:</strong> <?= htmlspecialchars($_SESSION['their-birthday']) ?></p>
            </div>
            <div class="father-info">
                <h3>Father's Information</h3>
            <?php if (isset($_SESSION['their-no_father']) && $_SESSION['their-no_father'] === 'yes'): ?>
                <p><strong>No Father's Name on Certificate:</strong> Yes</p>
            <?php else: ?>
                <p><strong>Father's First Name:</strong> <?= htmlspecialchars($_SESSION['their-father-first-name']) ?></p>
                <p><strong>Father's Middle Name:</strong> <?= htmlspecialchars($_SESSION['their-father-middle-name']) ?></p>
                <p><strong>Father's Last Name:</strong> <?= htmlspecialchars($_SESSION['their-father-last-name']) ?></p>
                <?php endif; ?>
            </div>
            <div class="mother-info">
                <h3>Mother's Information</h3>
                <p><strong>Mother's Marital Status:</strong> <?= htmlspecialchars($_SESSION['their-mother-marital-status']) ?></p>
                <p><strong>Mother's First Name:</strong> <?= htmlspecialchars($_SESSION['their-mother-first-name']) ?></p>
                <p><strong>Mother's Maiden Middle Name:</strong> <?= htmlspecialchars($_SESSION['their-mother-middle-name']) ?></p>
                <p><strong>Mother's Maiden Last Name:</strong> <?= htmlspecialchars($_SESSION['their-mother-last-name']) ?></p>
            </div>
            <div class="birth-place-info">
                <h3>Birth Place</h3>
                <p><strong>Birth Country:</strong> <?= htmlspecialchars($_SESSION['their-birth_country']) ?></p>
                <p><strong>Birth Province:</strong> <?= htmlspecialchars($_SESSION['their-birth_province']) ?></p>
                <p><strong>Birth Municipal:</strong> <?= htmlspecialchars($_SESSION['their-birth_municipal']) ?></p>
            </div>
            <div class="purpose">
                <h3>Purpose of Request</h3>
                <p><strong>Your Purpose for this Request:</strong> <?= htmlspecialchars($_SESSION['their-selectPurpose']) ?></p>
            </div>
      ...
      <script>
        ...
        const theirSex = <?= json_encode($_SESSION['their-sex']) ?>;
        const theirFirstName = <?= json_encode($_SESSION['their-first-name']) ?>;
        const theirMiddleName = <?= json_encode($_SESSION['their-middle-name']) ?>;
        const theirLastName = <?= json_encode($_SESSION['their-last-name']) ?>;
        const theirMartialStats = <?= json_encode($_SESSION['their-marital-status']) ?>;
        const theirMarriedLastName = <?= json_encode($_SESSION['their-married-last-name']) ?>;
        const theirBirthday = <?= json_encode($_SESSION['their-birthday']) ?>;

        const noFather = <?= json_encode($_SESSION['their-no_father'] ?? 'no') ?>;
        const fatherFirst = <?= json_encode($_SESSION['their-father-first-name'] ?? '') ?>;
        const fatherMiddle = <?= json_encode($_SESSION['their-father-middle-name'] ?? '') ?>;
        const fatherLast = <?= json_encode($_SESSION['their-father-last-name'] ?? '') ?>;

        const motherStatus = <?= json_encode($_SESSION['their-mother-marital-status']) ?>;
        const motherFirst = <?= json_encode($_SESSION['their-mother-first-name']) ?>;
        const motherMiddle = <?= json_encode($_SESSION['their-mother-middle-name']) ?>;
        const motherLast = <?= json_encode($_SESSION['their-mother-last-name']) ?>;

        const birthCountry = <?= json_encode($_SESSION['their-birth_country']) ?>;
        const birthProvince = <?= json_encode($_SESSION['their-birth_province']) ?>;
        const birthMunicipal = <?= json_encode($_SESSION['their-birth_municipal']) ?>;

        const purpose = <?= json_encode($_SESSION['their-selectPurpose']) ?>;
