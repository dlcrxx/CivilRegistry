...
<body>
    ...
    <div class="review-container">
        ...
        <div class="review-content">
            ...
            <div class="husband-info">
                <h3>Husband Information</h3>
                <p><strong>First Name:</strong> <?= htmlspecialchars($_SESSION['husband-first-name']) ?></p>
                <p><strong>Middle Name:</strong> <?= htmlspecialchars($_SESSION['husband-middle-name']) ?></p>
                <p><strong>Last Name:</strong> <?= htmlspecialchars($_SESSION['husband-last-name']) ?></p>
            </div>
             <div class="wife-info">
                <h3>Wife Information</h3>
                <p><strong>First Name:</strong> <?= htmlspecialchars($_SESSION['wife-first-name']) ?></p>
                <p><strong>Middle Name:</strong> <?= htmlspecialchars($_SESSION['wife-middle-name']) ?></p>
                <p><strong>Last Name:</strong> <?= htmlspecialchars($_SESSION['wife-last-name']) ?></p>
            </div>
            <div class="marriage-date">
                <h3>Marriage Date</h3>
                <p><strong>Date of Marriage:</strong> <?= htmlspecialchars($_SESSION['dateOfMarriage']) ?></p>
            </div>
            <div class="birth-place-info">
                <h3>Marriage Place</h3>
                <p><strong>Marriage Country:</strong> <?= htmlspecialchars($_SESSION['marriageCountry']) ?></p>
                <p><strong>Marriage Province:</strong> <?= htmlspecialchars($_SESSION['marriageProvince']) ?></p>
                <p><strong>Marriage Municipal:</strong> <?= htmlspecialchars($_SESSION['marriageMunicipal']) ?></p>
            </div>
            <div class="purpose">
      ...
      <script>
        ...
        const husbandFirst = <?= json_encode($_SESSION['husband-first-name'] ?? '') ?>;
        const husbandMiddle = <?= json_encode($_SESSION['husband-middle-name'] ?? '') ?>;
        const husbandLast = <?= json_encode($_SESSION['husband-Last-name'] ?? '') ?>;


        const wifeFirst = <?= json_encode($_SESSION['wife-first-name']) ?>;
        const wifeMiddle = <?= json_encode($_SESSION['wife-middle-name']) ?>;
        const wifeLast = <?= json_encode($_SESSION['wife-last-name']) ?>;

        const dateOfMarriage = <?= json_encode($_SESSION['dateOfMarriage']) ?>;

        const marriageCountry = <?= json_encode($_SESSION['marriageCountry']) ?>;
        const marriageProvince = <?= json_encode($_SESSION['marriageProvince']) ?>;
        const marriageMunicipal = <?= json_encode($_SESSION['marriageMunicipal']) ?>;
